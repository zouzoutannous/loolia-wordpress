...
class Themify_Enqueue_Assets {
	...
	private static $removeIndicator='/*NEED_UPDATE*/';
    ...
    public static function remove_default_js( $scripts ) {
		if ( ! themify_check( 'setting-jquery-migrate', true ) && ! themify_is_login_page() ) {
    ...
    public static function wp_footer() {
		...
		if (self::$concateFile !== null) {
			...
			if ($key !== '') {
				...
				$regenerate=false;
				if($exist===true && ($fd= fopen(self::$concateFile, 'r'))){
					flock($fd,LOCK_EX);
					$d = fgets($fd);
					flock($fd, LOCK_UN);
					fclose($fd);
					$exist=trim($d)!==self::$removeIndicator;
					unset($fd,$d);
					$regenerate=true;
				}	
				if ($exist===false) {
					...
					if($key !== ''){
						clearstatcache();
						if(empty( self::$concateFile ) || (($regenerate===true || !is_file(self::$concateFile)) && !file_put_contents(self::$concateFile,$str, LOCK_EX))){
							$key='';
						}
					}
					$str=null;
			...
			if($mobileDir!==null){
				...
				if($key!==''){
					...
					if ($regenerate===true || !is_file($mobileDir)) {
						...
						if($mobileDir!==null){
							clearstatcache();
							if (($regenerate===true || !is_file($mobileDir)) && !file_put_contents($mobileDir, $str, LOCK_EX ) ) {//maybe another proccess has already created it?
								$mobileDir=null;
							}
						}
						$str=null;
	...
	public static function clearConcateCss($blog_id=false){
		...
		if (Themify_Filesystem::is_dir($dir) && ($handle = opendir($dir))) {
			while (false !== ($f = readdir($handle))) {
				if ($f !== '.' && $f !== '..' && ($fd= fopen($dir.$f, 'r+'))) { 
					flock($fd, LOCK_SH|LOCK_EX);
					if( trim(fgets($fd))!==self::$removeIndicator){
						flock($fd, LOCK_UN);
						file_put_contents($dir.$f,self::$removeIndicator.PHP_EOL.file_get_contents($dir.$f),LOCK_EX);
					}
					else{
						flock($fd, LOCK_UN);
					}
